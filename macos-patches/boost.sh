...
downloadBoost()
{
    if [ ! -s "$BOOST_TARBALL" ]; then
        ...
        wget https://dl.bintray.com/boostorg/release/${BOOST_VERSION}/source/boost_${BOOST_VERSION2}.tar.gz
        doneSection
    fi
}
...
unpackBoost()
{
    ...
    [ -d "$BOOST_SRC" ] || ( cd "$SRCDIR"; gunzip -c "$BOOST_TARBALL" | tar xopf -
    )
...
BOOST_TARBALL="$CURRENT_DIR/boost_$BOOST_VERSION2.tar.gz"
